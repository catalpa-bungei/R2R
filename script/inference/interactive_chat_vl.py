...
from r2r.models.dynamic_sglang_selector_guard import DynamicSimpleSGLangSelectorGuard
import yaml
...
def run_dynamic_sglang_mode(args, model_config: dict, router_config: dict):
    ...
    ref_model_name = str(model_config.get("reference", {}).get("model_name", "")).lower()
    ref_model_path_lower = str(ref_model_path).lower()
    is_guard_model = "guard" in ref_model_name or "guard" in ref_model_path_lower
    effective_tp_size = model_config.get("reference", {}).get("tp_size", args.tp_size)
    if is_guard_model and effective_tp_size != 1:
        print(f"Guard model detected: overriding reference TP size from {effective_tp_size} to 1.")
        effective_tp_size = 1
    print(f"Using effective reference TP size: {effective_tp_size}")
    ...
    selector_cls = DynamicSimpleSGLangSelectorGuard if is_guard_model else DynamicSimpleSGLangSelector
    print(f"Using selector: {selector_cls.__name__}")

    generator = selector_cls(
