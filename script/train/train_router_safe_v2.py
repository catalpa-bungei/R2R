...
def _collect_string_values(obj: Any) -> List[str]:
    """Collect nested string values from dict/list configs for heuristic matching."""
...
def _should_enable_guard_safety_routing(config: Dict[str, Any]) -> bool:
    ...
    if "enabled" in safety_router_config:
        return bool(safety_router_config["enabled"])

    haystack = " ".join(_collect_string_values(config)).lower()
    return TARGET_ROUTER_PAIR_NAME in haystack


def _build_guard_safety_config(config: Dict[str, Any]) -> Dict[str, Any]:
    safety_router_config = copy.deepcopy(config.get("safety_router", {}))
    enabled = _should_enable_guard_safety_routing(config)
    safety_router_config["enabled"] = enabled
    ...
    safety_router_config.setdefault("general_step3_safe_column", "small_safe")
    safety_router_config.setdefault("safety_step3_safe_column", "reference_safe")
    safety_router_config.setdefault("reuse_cached_four_signal", False)
    ...
    safety_router_config.setdefault("recompute", False)

    return safety_router_config


def _normalize_local_dataset_path(dataset_path: Union[str, List[str]]) -> Optional[str]:
    if isinstance(dataset_path, list):
        if not dataset_path:
            return None
        dataset_path = dataset_path[0]
    if isinstance(dataset_path, str) and dataset_path.startswith("local:"):
...
def _build_assistant_content(record: Dict[str, Any]) -> str:
    ...
    if isinstance(reasoning, str) and reasoning.strip() and isinstance(response, str) and response.strip():
        return f"<think>\n{reasoning.strip()}\n</think>\n\n{response.strip()}"
...
def _resolve_guard_source_dataset_path(dataset_path: Union[str, List[str]], relative_path: str) -> Optional[str]:
    ...
    source_dataset_path = os.path.abspath(os.path.join(local_dataset_path, relative_path))
...
def _load_guard_source_records(dataset_path: Union[str, List[str]], relative_path: str) -> tuple[Dict[int, Dict[str, str]], Optional[str]]:
    source_dataset_path = _resolve_guard_source_dataset_path(dataset_path, relative_path)
    if not source_dataset_path:
        return {}, None

    source_dataset = load_from_disk(source_dataset_path)
    id_column = "id" if "id" in source_dataset.column_names else "data_id"
    if id_column not in source_dataset.column_names:
        return {}, source_dataset_path

    source_records: Dict[int, Dict[str, str]] = {}
    for record in source_dataset:
        try:
            data_id = int(record[id_column])
        except (TypeError, ValueError):
            continue
        source_records[data_id] = {
            "question": record.get("question") or record.get("input_text") or "",
            "assistant_content": _build_assistant_content(record),
        }
    return source_records, source_dataset_path
...
class Qwen3GuardSafetyAnnotator:
    """Annotate token datasets with Qwen3Guard-Stream safe probabilities."""
    ...
    def _score_input_ids(self, input_ids: List[int]) -> List[float]:
        if not input_ids:
            return []
        ...
        with torch.inference_mode():
            ...
            logits = outputs.risk_level_logits.squeeze(0).float()
            safe_probs = torch.softmax(logits, dim=-1)[:, SAFE_RISK_INDEX]

        return safe_probs.detach().cpu().tolist()

    def _build_conversation_ids(self, question: str, assistant_content: str) -> List[int]:
        messages = [
    ...
    def score_token_group(
        self,
        token_ids: List[int],
        source_record: Optional[Dict[str, str]] = None,
    ) -> tuple[List[float], str]:
        token_ids = [int(token_id) for token_id in token_ids]

        if source_record and source_record.get("assistant_content"):
            conversation_ids = self._build_conversation_ids(
                source_record.get("question", ""),
                source_record["assistant_content"],
            )
            safe_probs = self._score_input_ids(conversation_ids)
            match_start = _find_last_subsequence(conversation_ids, token_ids)
            if match_start is not None:
                match_end = match_start + len(token_ids)
                return safe_probs[match_start:match_end], "conversation_alignment"
            if len(safe_probs) >= len(token_ids):
                return safe_probs[-len(token_ids):], "conversation_suffix_fallback"

        return self._score_input_ids(token_ids), "token_only_fallback"


def annotate_dataset_with_guard_scores(
    dataset,
    split_name: str,
    dataset_path: Union[str, List[str]],
    safety_router_config: Dict[str, Any],
    annotator: Optional[Qwen3GuardSafetyAnnotator],
    label_column: str,
):
    score_column = safety_router_config["score_column"]
    ...
    if (
        ...
        and safety_router_config.get("reuse_cached_four_signal", False)
        and not safety_router_config.get("recompute", False)
        and route_label_column in dataset.column_names
        and safety_router_config["general_safe_column"] in dataset.column_names
        and safety_router_config["safety_safe_column"] in dataset.column_names
    ):
    ...
    if routing_logic != "four_signal" and annotator is None:
        raise ValueError("Qwen3Guard-Stream annotator is required outside four-signal Step 3 safety mode.")

    if (
        routing_logic != "four_signal"
        and ("data_id" not in dataset.column_names or response_token_column not in dataset.column_names)
    ):
        raise ValueError(
            f"Guard safety routing requires 'data_id' and '{response_token_column}' columns, "
            f"but {split_name} only has {dataset.column_names}"
        )
    ...
    if routing_logic != "four_signal":
        ...
        if source_dataset_path:
            ...
            print(f"No guard source dataset found for {split_name}; falling back to token-only moderation.")

    # Reset formatting before column mutation.
    dataset.set_format(type=None)

    num_rows = len(dataset)
    mask_values = [int(x) for x in dataset["mask"]] if "mask" in dataset.column_names else [1] * num_rows
    existing_labels = [int(x) for x in dataset[label_column]]

    data_ids = [int(x) for x in dataset["data_id"]] if "data_id" in dataset.column_names else [0] * num_rows
    token_positions = (
        [int(x) for x in dataset["token_id"]]
        if "token_id" in dataset.column_names
    ...
    if routing_logic == "four_signal":
        ...
        general_step3_safe_column = safety_router_config["general_step3_safe_column"]
        safety_step3_safe_column = safety_router_config["safety_step3_safe_column"]
        ...
        missing_columns = [
            ...
            for column in [
                ...
                general_step3_safe_column,
                safety_step3_safe_column,
            ]
            if column not in dataset.column_names
        ]
        if missing_columns:
            raise ValueError(f"Four-signal Step 3 safety routing requires missing columns: {missing_columns}")

        general_step3_safe_values = [int(x) for x in dataset[general_step3_safe_column]]
        safety_step3_safe_values = [int(x) for x in dataset[safety_step3_safe_column]]
        general_safe_flags = general_step3_safe_values
        safety_safe_flags = safety_step3_safe_values
        general_safe_scores = [float(value) for value in general_safe_flags]
        safety_safe_scores = [float(value) for value in safety_safe_flags]
        ...
        for mask, general_safe, general_correct, safety_safe, safety_correct in zip(
            ...
            if mask != 1:
                ...
                final_labels.append(0)
            elif general_safe == 0 and general_correct == 0:
                final_labels.append(1)
            elif general_safe == 0 and general_correct == 1 and safety_safe == 1 and safety_correct == 1:
                final_labels.append(1)
            elif (
                general_safe in [0, 1]
                and general_correct in [0, 1]
                and safety_safe in [0, 1]
                and safety_correct in [0, 1]
            ):
                final_labels.append(0)
            else:
                final_labels.append(-1)
        ...
        dataset = dataset.add_column(general_score_column, general_safe_scores)
        dataset = dataset.add_column(safety_score_column, safety_safe_scores)
        ...
        print(
            f"Four-signal Step 3 safety routing applied to {split_name}: "
        ...
        print(
            f"Safety flags read from {general_step3_safe_column} and "
            f"{safety_step3_safe_column}; Qwen3Guard-Stream thresholding was not used."
        )
        return dataset
    ...
    if score_column in dataset.column_names:
...
def main(config: dict, use_wandb: bool = False, validate_model_path: Optional[str] = None):
    ...
    safety_router_config = _build_guard_safety_config(config)
    config["safety_router"] = safety_router_config
    ...
    if safety_router_config["enabled"]:
        guard_device = config["training"]["params"].get("device", "cuda")
        test_dataset_path_for_guard = (
            data_config["test"]["path"]
            if (not "split_test_from_train" in data_config["test"]) or (not data_config["test"]["split_test_from_train"])
            else data_config["train"]["path"]
        )
        if safety_router_config.get("routing_logic") == "four_signal":
            print(
                "Enabling four-signal safety routing from Step 3 columns "
                f"{safety_router_config['general_step3_safe_column']} and "
                f"{safety_router_config['safety_step3_safe_column']}; "
                "Qwen3Guard-Stream thresholding is disabled in train_router_safe_v2.py."
            )
            guard_annotator = None
        else:
            print(
                "Enabling Qwen3Guard-Stream safety routing with "
                f"safe_prob threshold={safety_router_config['safe_probability_threshold']:.2f}"
            )
            guard_annotator = Qwen3GuardSafetyAnnotator(
                model_path=safety_router_config["guard_model_path"],
                device=guard_device,
            )
        datasets["train"] = annotate_dataset_with_guard_scores(
            dataset=datasets["train"],
            split_name="train",
            dataset_path=data_config["train"]["path"],
            safety_router_config=safety_router_config,
            annotator=guard_annotator,
            label_column=label_column,
        )
        datasets["test"] = annotate_dataset_with_guard_scores(
    ...
    label_column = training_config["loss"].get("label_column", label_column)
